...
private let FTImageGridViewImageMargin : CGFloat = 5.0
...
//        backgroundView.addSubview(tabBar)
        ...
        if let img : UIImage = (images[atIndex]).image {
            ...
            beginAnimationView.sd_setImage(with: URL(string: imageURL), completed: nil)
        }
...
public class FTImageGridView: UIView {
    ...
    public func showWithImageArray(_ imageArray : [String] , tapBlock : @escaping ((_ buttonsArray: [UIButton] , _ buttonIndex : NSInteger) ->())) {
        ...
        if imageArray.count > 0 {
            ...
            let imgHeight : CGFloat = (self.bounds.size.width - FTImageGridViewImageMargin * 3) / 4
            for i in 0 ... imageArray.count-1 {
                let x = CGFloat(i % 4) * (imgHeight + FTImageGridViewImageMargin)
                let y = CGFloat(i / 4) * (imgHeight + FTImageGridViewImageMargin)
                ...
                imageButton.sd_setImage(with: URL(string: imageArray[i]), for: .normal, completed: nil)
    ...
    public class func getHeightWithWidth(_ width: CGFloat, imgCount: Int) -> CGFloat{
        var photoAlbumHeight:CGFloat = 0.0
        if imgCount != 0 {
            let imgHeight: CGFloat = (width - FTImageGridViewImageMargin * 3) / 4
            photoAlbumHeight = imgHeight * CGFloat(ceilf(Float(imgCount) / 4)) + FTImageGridViewImageMargin * CGFloat(ceilf(Float(imgCount) / 4)-1)
        }
